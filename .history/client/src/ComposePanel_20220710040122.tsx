...
import { CreateTweetMutation, useCreateTweetMutation } from './generated/graphql';
...
const ComposePanel: React.FC<ComposePanelProps> = ({ currentUser }) => {
  async function createNewTweet(userId: string, body: string) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await useCreateTweetMutation[0]<CreateTweetMutation>({

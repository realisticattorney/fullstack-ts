...
const ComposePanel: React.FC<ComposePanelProps> = ({ currentUser }) => {
  function createNewTweet(userId: string, body: string) {
    console.log('creating new tweet', { body, currentUser });
    useCreateTweetMutation({ userId, body: body });
  }

  const handleSubmit = a(e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const textarea = e.currentTarget.querySelector('textarea');
    if (!textarea) throw new Error('No textarea found');
    const body = textarea.value;
    createNewTweet(currentUser.id, body);
    textarea.value = '';
  };
  return (

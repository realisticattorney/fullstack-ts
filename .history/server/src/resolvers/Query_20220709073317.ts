...
const queryTwitterResolver: QueryResolvers<TwitterResolverContext> = {
  ...
  tweets: (
    ...
    { db, dbTweetToFavoriteCountMap, dbUserCache, dbTweetCache } //context. This is set in the new ApolloServer constructor and it will be present in every resolver. THESE ARE RETURNED BY THE CONTEXT OBJECT. db.getAllUsers() will simply return an array. not something that references to th
  ) => {

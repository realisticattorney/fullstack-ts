...
const tweetTwitterResolver: TweetResolvers<TwitterResolverContext> = {
  ...
  author(tweet, _, { dbUserCache, dbTweetCache }) {
    ...
    //although we could use it optionally to get the author when the tweet.author is queried from a Favorite, the thing is that we haven't exposed who favorited
    if (!dbTweet)
      throw new Error(

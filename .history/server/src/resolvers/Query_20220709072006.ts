...
const queryTwitterResolver: QueryResolvers<TwitterResolverContext> = {
  ...
  tweets: (
    ...
    db.getAllFavorites().forEach((favorite) => {
      ...
      const count = dbTweetToFavoriteCountMap[favorite.tweetId] || 0; //it will go through {userId: number, tweetId: number} and add 1 to a hashmap with the tweetId as the key and the count as the value, which will start at 

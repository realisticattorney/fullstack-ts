...
const queryTwitterResolver: QueryResolvers<TwitterResolverContext> = {
  ...
  tweets: (
    ...
    db.getAllFavorites().forEach((favorite) => {
      ...
      const count = dbTweetToFavoriteCountMap[favorite.tweetId] || 0; //it will go through {userId: number, tweetId: number} and if it finds a tweetId that matches the tweetId of the favorite, it will return the count of that favorite. If it doesn't find a match, it will return 0.

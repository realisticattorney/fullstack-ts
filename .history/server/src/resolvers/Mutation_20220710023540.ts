...
const mutationTwitterResolver: MutationResolvers<TwitterResolverContext> = {
  ...
  async createTweet(_parent, args, { dbTweetCache, db }) {
    ...
    //the database has a method to create tweets, that takes care of most fields (e.g. createAt, updatedAt, and id). Usually you want your database to be in charge of creating the id, but you can override it if you want.

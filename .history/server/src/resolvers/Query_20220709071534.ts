...
const queryTwitterResolver: QueryResolvers<TwitterResolverContext> = {
  ...
  tweets: (
    ...
    db.getAllFavorites().forEach((favorite) => {
      //we haven't define a Favorites entity but think of it as a many to many relationship between tweets and users. So think of it as a join table that holds a tweet id and a user id in each 

...
const tweetTwitterResolver: TweetResolvers<TwitterResolverContext> = {
  ...
  favorited(tweet, _, { dbUserCache, dbTweetCache }) {
    const dbTweet = dbTweetCache[tweet.id];
    if (!dbTweet)
      throw new Error(
        'Attempted to find Tweet.favorited, but the tweet was not found in dbTweetCache'
      );
    const dbUser = dbUserCache[dbTweet.userId];
    if (!dbUser)
      throw new Error(
        "Attempted to find Tweet.favorited, but the tweet's author (a User) was not found in dbUserCache"
      );
    return dbUser.favorites.some(f => f.tweetId === tweet.id);

...
const queryTwitterResolver: QueryResolvers<TwitterResolverContext> = {
  ...
  tweets: (
    ...
    db.getAllFavorites().forEach((favorite) => {
      //we haven't define a Favorites entity but think of it as a many to many relationship between tweets and users. throguh the favorites table, we can get the list of tweets that are favorited by a user.

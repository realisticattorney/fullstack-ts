...
const userTwitterResolver: UserResolvers<TwitterResolverContext> = {
  ...
  stats(user, _, { db }) { //the first argument is the parent. what's happening in terms of the flow of what's calling into what here, when you make that top level query for current user, if someone has requested stats, 
    return {
      followingCount: 123,

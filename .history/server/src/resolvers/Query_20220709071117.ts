...
const queryTwitterResolver: QueryResolvers<TwitterResolverContext> = {
  ...
  tweets: (
    ...
    db.getAllUsers().forEach((user) => {  //we'll getting all users and iterating them and putting them into a cache (memory cache)
      //in a realistic scenario, you'd retrieve your tweets, get the list of user ids asociated with the tweet.author of those tweets and then primer your cache with that subset of users instead of all users as we do here. But it's har

...
const userTwitterResolver: UserResolvers<TwitterResolverContext> = {
  stats(user, _, { db }) { //this resolver is for UserStats type, which returns following, follows and tweet count of a user. We got it separately from User because we only need it for the 
    return {
      followingCount: 123,

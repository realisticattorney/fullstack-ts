...
const SCHEMA = loadSchemaSync(GRAPHQL_SCHEMA_PATH, {
  ...
  loaders: [new GraphQLFileLoader()], //this loader is designed to parse .graphql files returning an js object
});

export async function createApolloServer( ///these are the arguments for the ApolloServer constructor
  //_db: Db, //_ prefix let us tell to ts that we won't be using it now
  db: Db,
  httpServer: Server, //actual http server node instance
  app: express.Application
): Promise<ApolloServer<ExpressContext>> {
  //graphql schema
  //Query entity is special. It's a type of request like GET in a REST API.
  //by default values are nullable (they can return null). You have to specify that they are not nullable.
  //so the doctors schedule it's something we want to be nullable (empty argument)
  //[Suggestion!]! outer ! means it has to return an array. inner ! means none of the elements in the array can be null (could they be undefined?).
